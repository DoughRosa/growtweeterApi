...
class ReplyController {
  public async create(req: Request, res: Response) {
    ...
    if (!findUser) {
      return res.status(400).json({
        success: false,
        msg: "You must be logged to reply.",
      });
    }
    ...
    try {
      ...
      return res.status(404).json({ success: false, msg: "Reply Was NOT Created" });
    } catch (error) {
  ...
  public async list(req: Request, res: Response) {
    try {
      ...
      return res.status(200).json({ success: true, msg: "Replies Listed", data: replies });
    } catch (error) {
